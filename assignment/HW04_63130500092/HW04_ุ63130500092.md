...
ใช้คำสั่ง select เพื่อเลือกดูเฉพาะ ชื่อสินค้า โดย filter ด้วยค่าของ Sales ที่มีค่ามากที่สุดในรายการ
```
...
group_by ด้วย city จากนั้น summarise โดยใช้ sum ในการหาผลรวมของยอด Sale ในแต่ละเมือง 
```
...
529 Yuma                 841.  
```
### 3. หาจำนวนลูกค้าทั้งหมดที่อยู่ในรายการ (ต้องเป็นลูกค้าที่ไม่ซ้ำ)
...
1         793
```
### 4.ในแต่ละปี มียอดขายสูงสุดและต่ำสุดอยู่ที่เท่าไหร่ และ ปีที่มียอดขายเฉลี่ยสูงสุดคือปีไหน
...
คำสั่งที่ใช้ตรวจสอบคือ `year(), dmy()` จากนั้นจึงใช้คำสั่ง group_by โดยทำการ group_by ด้วยปีที่เรา extract ออกมา หลังจากนั้นจึง summarise เพื่อหาค่า max min และ mean จาก Sales
```
superstore  %>% mutate(year = year(dmy(Order.Date))) %>% group_by(year) %>% summarise(max = max(Sales), min = min(Sales), mean = mean(Sales))
```
...
# A tibble: 4 x 4
   year    max   min  mean
  <dbl>  <dbl> <dbl> <dbl>
1  2015 22638. 0.852  246.
2  2016  6355. 0.984  224.
3  2017 17500. 0.836  237.
4  2018 14000. 0.444  222.
```
